...
const handleClerkWebhook = httpAction(async (ctx, req) => {
    ...
    switch(event.type){
        ...
        case "user.updated":
            ...
            await ctx.runMutation(internal.user.create,{username:`${event.data.first_name} ${event.data.last_name}`,imageUrl:event.data.image_url,clerkId:event.data.id,email:event.data.email_addresses[0].email_address})
            break
